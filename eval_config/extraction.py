...
@register_filter("chainRegex")
class ChainRegexFilter(Filter):
    """A sequence of Regex filters that applies their own regex patterns to the original input string. Their results are concatenated together, in the order they appear in `regex_patterns`.
    """
    ...
    def __init__(
        ...
        self.regex_funcs = [re.compile(rp) for rp in regex_patterns]
    ...
    def apply(self, resps: list[list[str]], docs: list[dict]) -> list[list[str]]:
        ...
        def filter_set(inst):
            ...
            for resp in inst:
                matches = []
                for regex_func in self.regex_funcs:
                    match = regex_func.findall(resp)
                    if match:
